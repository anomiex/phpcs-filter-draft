...
use Anomiex\IgnoreFile;
use Anomiex\IgnoreFile\InvalidPatternException;
use ArrayIterator;
...
class IgnoreFileTest extends TestCase {
	...
	public function provideCases() {
		$map = array(
			'nomatch' => array( 'ignored' => false, 'unignored' => false ),
			'ignored' => array( 'ignored' => true, 'unignored' => false ),
			'unignored' => array( 'ignored' => false, 'unignored' => true ),
			'unignored_gitnomatch' => array( 'ignored' => false, 'unignored' => true ),
		);

		$data = file_get_contents( __DIR__ . '/IgnoreFileTestData.js' );
	...
	public function testCasesGit( $patterns, $pathmap, $skip = false ) {
		...
		try {
			...
			foreach ( $pathmap as $path => $expect ) {
				...
				$actual = array( 'ignored' => false, 'unignored' => false );
				if ( '' !== $ret ) {
					$ret = explode( "\0", $ret );
					$this->assertSame( '.gitignore', $ret[0], "Testing $path" );
					$this->assertSame( $path, $ret[3], "Testing $path" );
					$actual = array( 'ignored' => '!' !== $ret[2][0], 'unignored' => '!' === $ret[2][0] );
				}
	...
	public function provideCasesGit() {
		...
		$map = array(
			...
			'ignored' => array( 'ignored' => true, 'unignored' => false ),
			'unignored' => array( 'ignored' => false, 'unignored' => true ),
			'unignored_gitnomatch' => array( 'ignored' => false, 'unignored' => false ),
		);
